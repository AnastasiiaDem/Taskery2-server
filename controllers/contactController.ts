...
export const sendRespond = async (
  req: express.Request,
  res: express.Response,
) => {
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    transporter.sendMail(mail_config, (err: Error, data: any) => {
      ...
      return res
        .status(200)
        .json({ message: "The email was sent successfully" });
    });
  });
};
...
export const getRequests = async (
  ...
  Request.find({}, function (err: Error, data: any) {
    if (data.length === 0 || err)
      return res.status(400).json({ message: "There is no any requests" });
